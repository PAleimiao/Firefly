---
title: "朋友被执行dd命令清零硬盘后，数据恢复的血泪实录"
published: 2026-08-23
description: "朋友的朋友发来一行dd命令说是清理坏道，朋友执行后硬盘直接被清零。这篇文章记录了整个事件的经过，以及机械硬盘和SSD的数据恢复方案。"
tags: ["Linux", "数据恢复", "踩坑", "硬盘", "安全"]
category: "技术"
draft: false
---

# 朋友被执行dd命令清零硬盘后，数据恢复的血泪实录

> **警告：本文包含真实的硬盘损坏场景和数据恢复方案，阅读前请确保你没有在执行任何危险命令。**

## 😱 事情经过

事情是朋友跟我说的。

那天晚上，他在折腾一台瘦客户机（就是那种巴掌大的小主机），想改成低功耗下载机。他的一位"朋友"看他折腾得热闹，发来一条消息：
...
他扫了一眼，心想：`dd` 命令熟啊，磁盘复制工具，以前用过。`if` 是输入，`of` 是输出，`bs` 是块大小。看起来就是把什么东西写到硬盘里，应该是修复类的操作吧？

**于是他想都没想，回车。**

终端开始输出进度，他还挺高兴，觉得对方在帮他优化硬盘。过了大概两分钟，突然觉得不对劲——这进度条怎么一直在走？而且速度怎么这么快？

定睛一看：

- `if=/dev/zero` —— 输入源是**零设备**，也就是源源不断输出 `0`
- `of=/dev/sda` —— 输出目标是**第一块硬盘**
- `bs=4M` —— 每次写4MB

**翻译成人话：它在把整个硬盘全部填成0。**

他当场按 `Ctrl+C`，但已经晚了。两分钟的 `dd` 足够把分区表、引导记录、文件系统的关键区域全部抹成渣。

重启。黑屏。再重启。还是黑屏。

**那台瘦客户机，变成了一块价值几百元的砖头。**

然后他来找我说这个事。我听完血压都上来了，给他科普了半天。😂

最惨的是，他那台瘦客户机里原来还装了**三个操作系统**，全没了。

---

## 🔍 为什么这条命令这么狠？

为了让他理解这条命令的杀伤力，我当时打了个比方：

> `rm -rf /` 相当于把房子里的家具全砸了，但房子结构还在。
> 
> `dd if=/dev/zero of=/dev/sda` 相当于把**地基和承重墙一起炸平**，房子直接变平地。
> 
> 这串指令的破坏力，是 `rm -rf /` 的 Pro Max 版。
...
`dd` 命令是Linux下的**磁盘级复制工具**，它操作的是**裸设备**（`/dev/sda` 是整个硬盘，不是某个分区）。一旦开始写入，它不会管硬盘上有什么，直接从第一个字节开始，一个扇区一个扇区地填0。
...
如果硬盘有物理坏道，或者想更安全地操作，先用 `ddrescue` 做全盘镜像，然后在镜像上操作。
...
最后，我想对那位发命令的"朋友"说：**请你吃顿好的赔罪吧，至少两顿。** 😂
