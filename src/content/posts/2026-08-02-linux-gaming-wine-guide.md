...
> ⚠️ **先声明三件事，看完再动手，别怪我没提醒你。**

### 1. 带脑子操作

本文所有命令都是**示例**，不是让你无脑复制粘贴的。你的系统环境、硬件配置、发行版版本可能跟我不一样，**遇到报错先自己想想为啥，别一上来就截图甩我**。学会看终端输出里的关键词，Google 一下，80% 的问题你自己就能解决。

**复制粘贴前，问自己三个问题：**
- 这条命令是干啥的？（不知道就搜）
- 我的系统跟作者一样吗？（Arch 的命令别往 Ubuntu 上硬套）
- 出错了怎么办？（先看报错信息，别急着问）

### 2. 海外资源需要梯子

WineHQ 官方源、Lutris 官网、Proton GE GitHub Release、部分游戏的下载服务器……这些**都在海外**。如果你 `apt update` 卡死、`wget` 下载不动、GitHub 打不开，**大概率是网络问题，自个开梯子**。

> 🔗 **梯子安装教程**：后续会写一篇 Clash / 其他代理工具的安装配置文章，到时候会把链接补在这里。本文先不展开，免得跑题。

**订阅链接自己上网搜、自己买，本文不打广告、不宣传任何机场/节点**，别问我 "哪个好用"，问就是不知道。

### 3. 免责声明

折腾 Wine 和 Linux 游戏有一定风险，包括但不限于：系统变卡、Wine 前缀爆炸、游戏存档丢失、显卡驱动冲突。**操作前建议备份重要数据**，出了问题别找我赔 (￣▽￣)
